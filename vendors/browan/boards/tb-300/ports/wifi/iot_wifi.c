...
#if defined(CONFIG_IOT_DEVICE_SKU_TB_300)
...
#if defined(CONFIG_IOT_DEVICE_SKU_TB_300)
static SemaphoreHandle_t WiFi_Get_IP;
#endif
...
#if defined(CONFIG_IOT_DEVICE_SKU_TB_300)
    wifi_manager_event_handler(ctx, event);
#endif
...
{
#if defined(CONFIG_IOT_DEVICE_SKU_TB_300)
    xSemaphoreTake(WiFi_Get_IP, 10/portTICK_PERIOD_MS);
#endif
    return eWiFiSuccess;
}
/*-----------------------------------------------------------*/
...
#if defined(CONFIG_IOT_DEVICE_SKU_TB_300)
    WiFi_Get_IP = xSemaphoreCreateBinary();
...
{
    ESP_LOGI(TAG, "Waiting WiFi IP...");
    if(xSemaphoreTake(WiFi_Get_IP, portMAX_DELAY) == pdTRUE)
    {
        xSemaphoreGive(WiFi_Get_IP);
        return eWiFiSuccess;
    }
...
#endif //CONFIG_IOT_DEVICE_SKU_TB_300
/*-----------------------------------------------------------*/
